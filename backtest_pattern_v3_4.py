# -*- coding: utf-8 -*-
"""
前瞻性收益释放购买策略 — 回测验证程序 V3.4 (DL参数扫描版)

基于 V3.3, 自动扫描DL参数找最优组合:

V3.3 结果:
  - Mode D 年化5.9117%, 超越Mode B 5.8953% (+0.016%)
  - 夏普2.2689 vs 2.2444 (+1.1%)
  - 胜率92.7% vs 92.6%
  - DL加成+降级机制有效, 但alpha增量较小

V3.4 改进:
  1. 自动扫描DL参数: DL_MULT_WEIGHT × DL_DOWNGRADE_FACTOR
  2. 跳过Mode A (固定基线), 仅运行 Mode B(1次) + Mode D(多次)
  3. 每组参数只运行Mode D, 输出全部结果对比表
  4. 扫描范围:
     - DL_MULT_WEIGHT: [0.10, 0.20, 0.30, 0.40, 0.50]
     - DL_DOWNGRADE_FACTOR: [0.85, 0.90, 0.95, 1.00]
     - DL_MIN_ENTRY_QUALITY: [0.20, 0.30, 0.40]
  5. 共 5×4×3 = 60 组 (但DL只训练一次, 复用信号)

目标: 从Mode B 5.8953%基线最大化提升
...
OUTPUT_PATH = os.path.join(BASE_DIR, "前瞻策略回测报告_v3_4_sweep.xlsx")

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('backtest_pattern_v3_4.log', encoding='utf-8'),
...
# ===== [DL BLEND] DL混合参数 (V3.4: 可调参数, 供扫描) =====
DL_MULT_WEIGHT = 0.20           # 乘法加成系数 (扫描范围: 0.10~0.50)
DL_MAX_BOOST = 0.30             # 最大加成幅度 (略高于V3.3)
DL_MIN_ENTRY_QUALITY = 0.30     # 加成门槛 (扫描范围: 0.20~0.40)
DL_MIN_CONFIDENCE = 0.20        # 置信度门槛 (略低于V3.3)
DL_DOWNGRADE_THRESHOLD = 0.10   # 降级门槛
DL_DOWNGRADE_FACTOR = 0.95      # 降级幅度 (扫描范围: 0.85~1.00)
DL_MAX_VAL_LOSS = 0.50          # val_loss门控
DL_RETRAIN_INTERVAL = 30        # DL重训间隔


class DLAlphaManager:
    """[DL BLEND V3.4] Walk-forward GPU DL预测管理器 + 信号缓存

    V3.4 改进:
    - signal_cache: 按训练日期缓存alpha_signals + val_loss
    - replay模式: 跳过GPU训练, 直接从缓存读取信号
    - 首次运行后可无限次重放, 每次~0s (vs 重训~40s/cycle)
    """

    def __init__(self, replay_cache=None):
        self.engine = None
        self.alpha_signals = {}
        self.last_train_date = None
        self.last_val_loss = 999.0
        self.available = False
        self.signal_cache = {}      # {as_of_date: (signals, val_loss)}
        self.replay_mode = replay_cache is not None

        if self.replay_mode:
            self.signal_cache = replay_cache
            self.available = True
            logger.info(f"[DL V3.4] 回放模式, {len(replay_cache)}个缓存周期")
        else:
            try:
                ...
                if self.available:
                    logger.info("[DL V3.4] GPU可用, 训练+缓存模式")
                else:
                    logger.info("[DL V3.4] GPU不可用, 跳过DL")
            except ImportError as e:
                logger.warning(f"[DL V3.4] 导入失败: {e}")

    def should_retrain(self, current_date):
        if not self.available:
            return False
        if self.replay_mode:
            # 回放模式: 如果cache中有此日期的信号, 就"重训"(实际是读缓存)
            return current_date in self.signal_cache or self.last_train_date is None
    ...
    def train_and_predict(self, as_of_date):
        if self.replay_mode:
            # 回放: 直接从缓存读取
            if as_of_date in self.signal_cache:
                self.alpha_signals, self.last_val_loss = self.signal_cache[as_of_date]
        ...
        try:
            t0 = time.time()
            logger.info(f"[DL V3.4] Walk-forward 训练 as_of={as_of_date}")
            ...
            # 缓存信号
            self.signal_cache[as_of_date] = (dict(self.alpha_signals), self.last_val_loss)

            reliable_tag = "可用" if self.is_reliable() else "不可靠,跳过"
            logger.info(f"[DL V3.4] 训练完成: {len(self.alpha_signals)}产品, "
            ...
            logger.error(f"[DL V3.4] 训练失败: {e}", exc_info=True)
            self.last_val_loss = 999.0
    ...
    def run_single_mode(self, mode, dl_alpha=None):
        ...
        def _rebuild_pat(date):
            ...
            if use_dl_blend and dl_alpha is not None and dl_alpha.should_retrain(date):
                ...
                # 诊断 (仅非回放模式打印, 避免扫描时刷屏)
                if not dl_alpha.replay_mode and dl_alpha.is_reliable() and dl_alpha.alpha_signals:
                    n_dl_total = len(dl_alpha.alpha_signals)
                    n_pat = len(patterns)
                    overlap = set(dl_alpha.alpha_signals.keys()) & set(patterns.keys())
                    dl_entries = [dl_alpha.alpha_signals[k].get('entry_quality', 0)
                                  for k in overlap]
                    n_above = sum(1 for e in dl_entries if e >= DL_MIN_ENTRY_QUALITY)
                    n_below = sum(1 for e in dl_entries if e < DL_DOWNGRADE_THRESHOLD)
                    logger.info(f"  [{date}] DL诊断: {n_dl_total}个DL信号, "
                                f"{n_pat}个pattern, {len(overlap)}个重叠, "
                                f"{n_above}个可加成(≥{DL_MIN_ENTRY_QUALITY}), "
                                f"{n_below}个可降级(<{DL_DOWNGRADE_THRESHOLD})")
            ...
            if use_dl_blend and dl_alpha is not None and dl_alpha.is_reliable():
                ...
                for key in list(predicted_scores.keys()):
                    ...
                    if dl_entry >= DL_MIN_ENTRY_QUALITY:
                        ...
                        n_dl_boosted += 1
                    elif dl_entry < DL_DOWNGRADE_THRESHOLD:
                        # 降级: DL强烈不同意pattern, 轻微降低排名
                        downgraded = old_score * DL_DOWNGRADE_FACTOR
                        predicted_scores[key] = (downgraded, predicted_scores[key][1])
                        n_dl_downgraded += 1
...
def main():
    ...
    流程:
    1. 加载数据, 运行Mode B基线 (1次)
    2. 运行Mode D训练DL并缓存信号 (1次, ~6分钟)
    3. 用缓存信号扫描DL参数 (60组, 每组~5秒, 共~5分钟)
    4. 输出全部结果排名
    """
    global ALPHA_预测加成, DL_MULT_WEIGHT, DL_MAX_BOOST
    global DL_MIN_ENTRY_QUALITY, DL_DOWNGRADE_FACTOR

    sep = '=' * 74
    print(sep)
    print("  前瞻策略回测 V3.4 — DL参数扫描")
    print(f"  ALPHA_预测加成 = 0.30 (固定)")
    print(f"  扫描: DL_MULT_WEIGHT × DL_DOWNGRADE_FACTOR × DL_MIN_ENTRY_QUALITY")
    print(sep)
    ...
    print(f"  Mode B: 年化 {rb['ann_ret']:+.4f}%  夏普 {rb['sharpe']:.4f}")

    # ② 训练DL并缓存信号 (仅运行1次)
    print("\n  [Phase 2] DL训练 + 信号缓存...")
    dl_train = DLAlphaManager()
    if not dl_train.available:
        print("  GPU不可用, 无法进行DL参数扫描。")
        return

    dl_train.train_and_predict(回测开始日)
    rd_warmup = engine.run_single_mode('D', dl_alpha=dl_train)
    print(f"  DL训练完成, 缓存了 {len(dl_train.signal_cache)} 个周期的信号")
    print(f"  默认参数: 年化 {rd_warmup['ann_ret']:+.4f}%  夏普 {rd_warmup['sharpe']:.4f}")

    signal_cache = dl_train.signal_cache

    # ③ 参数扫描
    print("\n  [Phase 3] DL参数扫描...")
    sweep_mult = [0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50]
    sweep_dg = [0.80, 0.85, 0.90, 0.95, 1.00]
    sweep_entry = [0.15, 0.20, 0.25, 0.30, 0.40]
    total = len(sweep_mult) * len(sweep_dg) * len(sweep_entry)
    print(f"  共 {total} 组参数 (每组~5秒)")

    results = []
    count = 0
    for mult_w in sweep_mult:
        for dg_f in sweep_dg:
            for min_e in sweep_entry:
                count += 1
                # 设置全局DL参数
                DL_MULT_WEIGHT = mult_w
                DL_MAX_BOOST = mult_w * 1.5   # 上限 = 加成系数 × 1.5
                DL_DOWNGRADE_FACTOR = dg_f
                DL_MIN_ENTRY_QUALITY = min_e

                # 创建回放模式的DL管理器
                dl_replay = DLAlphaManager(replay_cache=signal_cache)
                dl_replay.train_and_predict(回测开始日)

                # 运行Mode D (无DL训练, 仅使用缓存)
                rd = engine.run_single_mode('D', dl_alpha=dl_replay)

                delta = rd['ann_ret'] - rb['ann_ret']
                results.append({
                    'mult_w': mult_w, 'dg_f': dg_f, 'min_e': min_e,
                    'ann_ret': rd['ann_ret'], 'sharpe': rd['sharpe'],
                    'win_rate': rd['win_rate'], 'n_trades': rd['n_trades'],
                    'delta_vs_b': delta,
                })

                if count % 25 == 0 or count == total:
                    print(f"    进度: {count}/{total}  当前最优delta: "
                          f"{max(r['delta_vs_b'] for r in results):+.4f}%")

    # ④ 输出结果
    results.sort(key=lambda r: r['ann_ret'], reverse=True)

    print(f"\n{sep}")
    print("     V3.4 DL参数扫描结果 (按年化收益排序)")
    print(sep)
    print(f"  Mode B 基线: 年化 {rb['ann_ret']:+.4f}%  夏普 {rb['sharpe']:.4f}")
    print()
    print(f"  {'#':<4} {'MULT_W':<8} {'DG_F':<7} {'MIN_E':<7} {'年化%':<12} {'vs B':<10} {'夏普':<8} {'胜率%':<7} {'交易':<5}")
    print("  " + "-" * 70)

    n_better = 0
    for i, r in enumerate(results[:30]):  # Top 30
        tag = '★' if r['delta_vs_b'] > 0 else ' '
        if r['delta_vs_b'] > 0:
            n_better += 1
        print(f"  {tag}{i+1:<3} {r['mult_w']:<8.2f} {r['dg_f']:<7.2f} {r['min_e']:<7.2f} "
              f"{r['ann_ret']:+.4f}%   {r['delta_vs_b']:+.4f}%   {r['sharpe']:<8.4f} "
              f"{r['win_rate']:<7.1f} {r['n_trades']:<5d}")

    print()
    print(f"  超越Mode B的组合: {n_better}/{total}")

    best = results[0]
    print(f"\n  最优参数: MULT_W={best['mult_w']:.2f}, DG_F={best['dg_f']:.2f}, MIN_E={best['min_e']:.2f}")
    print(f"  最优结果: 年化 {best['ann_ret']:+.4f}%  夏普 {best['sharpe']:.4f}  vs B: {best['delta_vs_b']:+.4f}%")

    if best['delta_vs_b'] > 0:
        print(f"\n  结论: DL参数优化有效!")
        print(f"  建议写入 backtest_pattern_v3_5.py 使用最优参数:")
        print(f"    DL_MULT_WEIGHT = {best['mult_w']:.2f}")
        print(f"    DL_DOWNGRADE_FACTOR = {best['dg_f']:.2f}")
        print(f"    DL_MIN_ENTRY_QUALITY = {best['min_e']:.2f}")
    else:
        print(f"\n  结论: 所有DL参数组合均未超越Mode B")
