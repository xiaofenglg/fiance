...
# ============================================================
# 策略参数 - 量化优化版（目标年化5%+）
# 核心改动: 多因子Alpha评分 + 凯利仓位管理 + 趋势衰减卖出
# ============================================================
信号阈值_买入 = 3.0       # 买入: 当日年化>3.0%
信号阈值_卖出 = 2.0       # 卖出: 当日年化≤2.0%才卖
...
# Alpha评分权重（多因子排名）
W_SUCCESS = 0.50           # 成功率权重（核心因子）
W_AVG_RET = 0.25           # 平均收益权重
W_SHARPE  = 0.15           # 夏普比率权重（收益/波动）
W_PERSIST = 0.10           # 持续天数权重
...
初始资金 = 100_000_000       # 1亿
最大持仓数 = 9
单仓基础 = 初始资金 / 9      # 基准仓位~1111万（9仓更高覆盖率）
单仓最小倍率 = 1.0           # 固定仓位
单仓最大倍率 = 1.0           # 固定~1111万/仓
产品库重建间隔 = 10           # 每10天重建

# T+1 交易机制参数
最短持有交易日_赎回 = 4   # 4个交易日后可赎回
最大持有交易日 = 30        # 最多30个交易日
长假阈值_天 = 3            # 下一交易日间隔>此值视为长假, 不提交申购

# 趋势衰减卖出参数
衰减卖出_回看天数 = 3      # 检查最近3个收益日的趋势
衰减卖出_比例 = 0.0        # 0=禁用趋势衰减卖出（由硬阈值和最大持有控制）
...
class BacktestEngine:
    ...
    def build_library(self, as_of_date):
        """构建产品库 + 多因子Alpha评分
        仅使用 eval_end < as_of_date 的信号（禁止前瞻）
        Alpha = W_SUCCESS*norm(成功率) + W_AVG_RET*norm(平均收益)
              + W_SHARPE*norm(收益/波动) + W_PERSIST*norm(持续天数)
        """
        raw = {}
        for key, product in self.products.items():
            ...
            sharpe = avg_ret / max(std_ret, 0.01)  # 收益/波动
            raw[key] = {
        ...
        for key, info in raw.items():
            alpha = (W_SUCCESS * norm(info['success_rate'], sr_min, sr_max) +
    ...
    def _check_sells(self, date):
        ...
        for key in list(self.positions):
            ...
            # ③ 趋势衰减卖出: 当前收益大幅低于入场收益 → 释放窗口结束
            if ret is not None and pos.entry_return > 0:
                decay_threshold = pos.entry_return * 衰减卖出_比例
                if ret < decay_threshold and ret < 信号阈值_买入:
                    # 额外确认: 检查最近N天是否持续下行
                    idx = p.ret_date_idx.get(date)
                    if idx is not None and idx >= 衰减卖出_回看天数:
                        recent = [p.rets.get(p.ret_dates[idx - j], 0)
                                  for j in range(衰减卖出_回看天数)]
                        if all(r < 信号阈值_买入 for r in recent):
                            self.pending_sells.append((key, date,
                                f'趋势衰减({ret:.1f}%<入场{pos.entry_return:.1f}%×{衰减卖出_比例}持有{hold_td}交易日)'))
                            self.pending_sell_keys.add(key)
    ...
    def _compute_position_size(self, alpha_score):
        """基于Alpha评分的凯利仓位: 高分产品→大仓位，低分→小仓位"""
        # alpha ∈ [0,1] → 仓位倍率 ∈ [单仓最小倍率, 单仓最大倍率]
        ratio = 单仓最小倍率 + (单仓最大倍率 - 单仓最小倍率) * alpha_score
    ...
    def _find_and_queue_buys(self, date, slots, di):
        ...
        # 排名: Alpha评分优先（50%成功率 + 25%收益 + 15%夏普 + 10%持续）
        # 同分按当前收益排
        signals.sort(key=lambda x: (-x[1], -x[2]))

        # 计算当前已承诺的资金（持仓 + 待买）
        committed = sum(pos.amount for pos in self.positions.values())
        for pk, _, _ in self.pending_buys:
            committed += self._compute_position_size(
                self.library.get(pk, {}).get('alpha', 0.5))

        queued = 0
        for key, alpha, ret, sr, avg_r in signals[:slots]:
            size = self._compute_position_size(alpha)
            # 不超过可用现金（留一定余量）
            if self.cash - size < -单仓基础 * 0.3:
                continue

            reason = f'α{alpha:.2f} 成功率{sr}% 收益{ret:.1f}% 仓位{size / 1e4:.0f}万'
    ...
    def run(self):
        ...
        sep = '=' * 70
        print(f"\n{sep}")
        print("   V5.4量化策略回测 (多因子Alpha + 凯利仓位 + T+1)")
        print(f"   区间: {回测开始日} ~ {回测结束日}")
        print(f"   资金: {初始资金 / 1e8:.1f}亿 | "
              f"仓位: {最大持仓数}×({单仓基础*单仓最小倍率/1e4:.0f}~{单仓基础*单仓最大倍率/1e4:.0f})万")
        print(f"   Alpha: W_SR={W_SUCCESS} W_RET={W_AVG_RET} W_SH={W_SHARPE} W_PS={W_PERSIST}")
        print(f"   T+1: 持有{最短持有交易日_赎回}~{最大持有交易日}交易日 | "
              f"趋势衰减<{衰减卖出_比例*100:.0f}%入场值")
        print(sep)
    ...
    def _generate_report(self):
        ...
        rows = [
            ...
            ["持仓规则", f"最多{最大持仓数}个, {单仓基础*单仓最小倍率/1e4:.0f}~{单仓基础*单仓最大倍率/1e4:.0f}万/仓(凯利仓位)"],
