# -*- coding: utf-8 -*-
"""
前瞻性收益释放购买策略 — 回测验证程序 V3.3 (DL乘法加成 — 宽松门槛)

基于 V3.2, 调整DL门槛:

V3.2 结果:
  - Mode D 与 Mode B 完全相同 (5.8953%) — DL信号全被过滤
  - 原因: entry>=0.50 + conf>=0.50 对DL模型输出太严格
  - DL统计回退的confidence固定=0.3, 无法通过0.50门槛
  - DL模型的entry_quality多数产品也低于0.50

V3.3 修正:
  1. 降低门槛: entry>=0.30, conf>=0.25 (让DL信号通过)
  2. 保留乘法加成 (不用加法混合, 不创建DL-only预测)
  3. val_loss质量门控放宽到0.50
  4. 加成系数提高到0.20, 上限0.25
  5. 新增: DL降级机制 — 当DL entry_quality极低(<0.10)
     且pattern预测的产品, 略微降低其排名 (-5%)
  6. 新增: 诊断日志 — 每个rebuild周期记录DL加成数量

目标: 从Mode B 5.8953%基线提升
...
OUTPUT_PATH = os.path.join(BASE_DIR, "前瞻策略回测报告_v3_3_dl.xlsx")

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('backtest_pattern_v3_3.log', encoding='utf-8'),
...
# ===== [DL BLEND] DL混合参数 (V3.3: 宽松门槛 + 乘法加成 + 降级机制) =====
DL_MULT_WEIGHT = 0.20           # 乘法加成系数: score *= (1 + dl_boost * 此值)
DL_MAX_BOOST = 0.25             # 最大加成幅度
DL_MIN_ENTRY_QUALITY = 0.30     # DL entry_quality >= 此值才加成 (V3.2: 0.50, 太严)
DL_MIN_CONFIDENCE = 0.25        # DL confidence >= 此值才加成 (V3.2: 0.50, 太严)
DL_DOWNGRADE_THRESHOLD = 0.10   # entry_quality < 此值 → 降级 (DL认为不释放)
DL_DOWNGRADE_FACTOR = 0.95      # 降级幅度: score *= 此值 (轻微惩罚)
DL_MAX_VAL_LOSS = 0.50          # val_loss > 此值时跳过DL信号 (V3.2: 0.45)
...
class DLAlphaManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.available:
                logger.info("[DL Blend V3.3] GPU可用, 乘法加成模式")
            else:
                logger.info("[DL Blend V3.3] GPU不可用, 跳过DL")
        except ImportError as e:
            logger.warning(f"[DL Blend V3.3] 导入失败: {e}")
    ...
    def train_and_predict(self, as_of_date):
        ...
        try:
            t0 = time.time()
            logger.info(f"[DL Blend V3.3] Walk-forward 训练 as_of={as_of_date}")
            ...
            logger.info(f"[DL Blend V3.3] 训练完成: {len(self.alpha_signals)}产品, "
            ...
            logger.error(f"[DL Blend V3.3] 训练失败: {e}", exc_info=True)
            self.last_val_loss = 999.0
    ...
    def run_single_mode(self, mode, dl_alpha=None):
        ...
        def _rebuild_pat(date):
            ...
            if use_dl_blend and dl_alpha is not None and dl_alpha.should_retrain(date):
                ...
                # 诊断: 统计DL信号与pattern的重叠
                if dl_alpha.is_reliable() and dl_alpha.alpha_signals:
...
def main():
    """V3.3 主入口: 直接对比 Mode A / Mode B(0.30) / Mode D(B+DL乘法加成+降级)

    策略说明:
    - Mode A: 纯反应式基线 (无预测, 精确复现 backtest_v6)
    - Mode B: 基于周期+阶段规律预测的Alpha排名加成 (ALPHA=0.30, 原始最优)
    - Mode D: Mode B + DL乘法加成 + 降级机制
      * DL加成: entry>=0.30, score *= (1 + boost), 上限25%
      * DL降级: entry<0.10, score *= 0.95 (DL不看好的产品降权)
      * val_loss > 0.50 时跳过DL

    V3.2→V3.3改动:
      - 门槛降低: entry 0.50→0.30, conf 0.50→0.25
      - 新增降级机制
      - val_loss门控放宽 0.45→0.50
      - 加成系数提高 0.15→0.20, 上限0.20→0.25
    """
    global ALPHA_预测加成

    sep = '=' * 74
    print(sep)
    print("  前瞻策略回测 V3.3 — DL乘法加成+降级 (宽松门槛)")
    print(f"  ALPHA_预测加成 = 0.30 (V3最优, 固定)")
    print(f"  DL加成 = {DL_MULT_WEIGHT} (上限{DL_MAX_BOOST}), 降级={DL_DOWNGRADE_FACTOR}")
