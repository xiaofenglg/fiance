# -*- coding: utf-8 -*-
"""
前瞻性收益释放购买策略 — 回测验证程序 V3.1 (DL增强版)

基于 V3 (backtest_pattern_v2.py), 在Mode B/D的预测排名中混合DL深度学习信号:
  - DL模型 (gpu_alpha_v2.py) 学习12维特征, 预测未来5日释放概率
  - DL信号与现有周期+阶段预测混合, 增强排名置信度
  - 仅影响 predicted_scores, 不修改买卖阈值/库构建/任何交易逻辑

改动点 (相对v2):
  1. 新增 DLAlphaManager 类 — walk-forward GPU训练
  2. _find_buys() 中 predicted_scores 混合DL信号 (仅Mode D)
  3. _rebuild_pat() 中同步重训DL模型
  4. 新增 Mode D — Mode B + DL混合预测
  其余所有代码完全不变 (A/B/C模式完全保留)

目标: 从5.89%基线提升, 不降低风险调整指标
...
OUTPUT_PATH = os.path.join(BASE_DIR, "前瞻策略回测报告_v3_dl.xlsx")

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('backtest_pattern_v3.log', encoding='utf-8'),
...
# ===== [DL BLEND] DL混合参数 =====
DL_BLEND_WEIGHT = 0.30          # DL信号在混合中的权重 (pattern 70% + DL 30%)
DL_MIN_ENTRY_QUALITY = 0.35     # DL entry_quality >= 此值才混合
DL_MIN_CONFIDENCE = 0.3         # DL confidence >= 此值才混合
DL_NEW_SCORE_FLOOR = 0.15       # 纯DL预测(无pattern)的最低分数门槛
DL_RETRAIN_INTERVAL = 30        # DL重训间隔(天), 与规律学习同步


class DLAlphaManager:
    """[DL BLEND] Walk-forward GPU DL预测管理器"""
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.available:
                logger.info("[DL Blend] GPU可用, 将使用DL预测混合")
            else:
                logger.info("[DL Blend] GPU不可用, 跳过DL混合")
        except ImportError as e:
            logger.warning(f"[DL Blend] 导入失败: {e}")
    ...
    def train_and_predict(self, as_of_date):
        ...
        try:
            t0 = time.time()
            logger.info(f"[DL Blend] Walk-forward 训练 as_of={as_of_date}")
            self.alpha_signals = self.engine.train_and_predict(as_of_date=as_of_date)
            self.last_train_date = as_of_date
            elapsed = time.time() - t0
            val_loss = getattr(self.engine, '_last_avg_val_loss', 999)
            logger.info(f"[DL Blend] 训练完成: {len(self.alpha_signals)}产品, "
                        f"{elapsed:.1f}s, val_loss={val_loss:.3f}")
            gc.collect()
            try:
                import torch
            ...
            logger.error(f"[DL Blend] 训练失败: {e}", exc_info=True)
...
class PatternBacktestEngine:
    ...
    def run_single_mode(self, mode, dl_alpha=None):
        ...
        def _find_buys(date, slots, di):
            ...
            # === [DL BLEND] 混合DL预测信号 (仅Mode D) ===
            if use_dl_blend and dl_alpha is not None:
                ...
                for key in library:
                    ...
                    dl_score = dl_entry * 0.6  # 归一化DL分数到类似pattern范围

                    if key in predicted_scores:
                        # 两种预测都有 → 混合: pattern权重70% + DL权重30%
                        old_score = predicted_scores[key][0]
                        blended = old_score * (1 - DL_BLEND_WEIGHT) + dl_score * DL_BLEND_WEIGHT
                        predicted_scores[key] = (blended, predicted_scores[key][1])
                    elif dl_score >= DL_NEW_SCORE_FLOOR:
                        # 仅DL预测 → 创建合成预测对象
                        fake_pred = Prediction(
                            ...
                            source='dl',
                            td_until=3,
                            predicted_end_date=(pd.Timestamp(date) + pd.Timedelta(days=5)).strftime('%Y-%m-%d')
                        )
                        predicted_scores[key] = (dl_score, fake_pred)
...
def main():
    ...
    sep = '=' * 74
    print(sep)
    print("  前瞻策略回测 V3.1 — DL预测混合增强 (基于V3 Mode B最优)")
    print(f"  ALPHA_预测加成 = 0.30 (V3最优, 固定)")
    print(f"  DL混合权重 = {DL_BLEND_WEIGHT} (pattern {1-DL_BLEND_WEIGHT:.0%} + DL {DL_BLEND_WEIGHT:.0%})")
    ...
    # ④ 打印对比
    print(f"\n{sep}")
    print("     V3.1 DL混合增强 回测对比")
    print(sep)

    print(f"  {'指标':<20} {'A(反应)':<14} {'B(+预测)':<14} {'D(+DL混合)':<14}")
    print("  " + "-" * 58)
